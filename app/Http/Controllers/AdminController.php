...
class AdminController extends Controller
{
    ...
    public function updateTour(Request $request, Tour $tour): RedirectResponse
    {
        try {
            $validated = $request->validate([
                ...
                'schedule_title.*'       => 'nullable|string|max:255',
                'schedule_description.*' => 'nullable|string',
                // departures - Bỏ after:today để có thể cập nhật tour đã có
                'departure_date.*'  => 'nullable|date',
                'seats_total.*'     => 'nullable|integer|min:1|max:100',
                'seats_available.*' => 'nullable|integer|min:0|max:100',
                'price_dep.*'       => 'nullable|numeric|min:0',
        ...
        try {
            ...
            return redirect()->back()
                ->withInput()
                ->with('error', 'Có lỗi xảy ra khi cập nhật tour: ' . $e->getMessage());
        }
    }
    ...
    public function showBanner(\App\Models\Banner $banner): View
    {
    ...
    public function storeBanner(Request $request): RedirectResponse
    {
        try {
            ...
            // Xóa field image khỏi validated vì không cần lưu vào DB
            ...
            return redirect()->route('admin.banners')->with('success', 'Banner đã được tạo thành công!');
        } catch (\Exception $e) {
            return redirect()->back()
                ->withInput()
                ->withErrors(['error' => 'Lỗi khi tạo banner: ' . $e->getMessage()]);
        }
    }

    public function editBanner(\App\Models\Banner $banner): View
    {
        return view('admin.banners.edit', compact('banner'));
    }

    public function updateBanner(Request $request, \App\Models\Banner $banner): RedirectResponse
    {
        $validated = $request->validate([
            'title' => 'required|string|max:200',
            'description' => 'nullable|string',
    ...
    public function deleteBanner(\App\Models\Banner $banner): RedirectResponse
    {
        // Xóa ảnh nếu có
        if ($banner->image_url && strpos($banner->image_url, '/storage/') !== false) {
    ...
    public function moveBanner(Request $request, \App\Models\Banner $banner): RedirectResponse
    {
        $direction = $request->input('direction'); // 'up' or 'down'
        
        if ($direction === 'up') {
            $swapBanner = \App\Models\Banner::where('sort_order', '<', $banner->sort_order)
        ...
        if ($swapBanner) {
            ...
            $banner->update(['sort_order' => $swapBanner->sort_order]);
            $swapBanner->update(['sort_order' => $tempOrder]);
        }

        return redirect()->route('admin.banners')->with('success', 'Thứ tự banner đã được cập nhật!');
    }
}
